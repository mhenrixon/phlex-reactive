...
RSpec.describe "Cross-root compute mirror (issue #159)", type: :system do
  it "paints declared recap nodes OUTSIDE the reactive root, with no round trip" do
    visit "/new_order"
    expect(page).to have_css("#summary-cash", text: "0")
    expect(page).to have_css("#summary-total", text: "—") # unseeded: proves the paint below
    ...
    # …and the recap OUTSIDE the root repainted through the declared mirror:
